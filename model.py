...
class BiLSTMCRF(nn.Module):
    ...
    def init_hidden(self):
        a=torch.randn(2, self.batch_size, self.hidden_dim // 2)
        b=torch.randn(2, self.batch_size, self.hidden_dim // 2)
        if self.use_gpu:
            a=a.cuda()
            b=b.cuda()
        return (a,b)
    ...
    def __get_lstm_features(self, sentence):
        self.hidden = self.init_hidden()
#         print('sentence.shape[1]', sentence.shape)
...
#         print('lstm_out.shape', lstm_out.shape)
    ...
    def real_path_score(self, logits, label):
        ...
        '''
        score = torch.zeros(1)
        a=torch.tensor([self.tag_map[START_TAG]], dtype=torch.long)
        if self.use_gpu:
            a=a.cuda()
        label = torch.cat([a, label])
        if self.use_gpu:
            score=score.cuda()
            label=label.cuda()
    ...
    def __viterbi_decode(self, logits):
        ...
        if self.use_gpu:
            ...
            backpointers=backpointers.cuda()
