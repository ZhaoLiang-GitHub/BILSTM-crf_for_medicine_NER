...
class DataManager():
    def __init__(self, max_length=100, batch_size=20, data_type='train', tags=[]):
        ...
        if data_type == "train":
            ...
            self.data_path = "data/BIO_train.txt"
        elif data_type == "test":
            self.data_path = "data/BIO_test.txt"
            self.load_data_map()
        elif data_type == "test":
...
#                     print('@',word,tag)
                    ...
                    print('False',type(line),line,ord(line[0]))
                    continue
...
#         print('len(data)',len(data))
        while True:
            ...
            if index+self.batch_size >= len(self.data):#self.data包含[sentence, tag]
                pad_data = self.pad_data(self.data[-self.batch_size:])#对sentence tag都进行补0
                # print('pad_data1', len(pad_data), len(pad_data[0])
                self.batch_data.append(pad_data)
                break
            else:
                pad_data = self.pad_data(self.data[index:index+self.batch_size])
                index += self.batch_size
                # print('pad_data2', pad_data)
                self.batch_data.append(pad_data)
